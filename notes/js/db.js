...
async function _flushOfflineQueue() {
  ...
  // Re-sync notes from Supabase → refresh IDB and the in-memory cache
  try {
    const nR = await _doFetch('GET', `${SUPABASE_URL}/rest/v1/notes?select=*&order=created_at.desc`);
    if (nR.data) {
      IDB.replaceAll('notes', nR.data).catch(() => {});
      window._notesDocs = nR.data;
      if (typeof initNotesEntries === 'function') initNotesEntries(nR.data);
