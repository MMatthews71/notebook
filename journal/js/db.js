...
async function _flushOfflineQueue() {
  ...
  // Re-sync journal entries from Supabase → refresh IDB and the UI
  try {
    const jR = await _doFetch('GET', `${SUPABASE_URL}/rest/v1/journal_entries?select=*&order=created_at.desc`);
    if (jR.data) IDB.replaceAll('journal_entries', jR.data).catch(() => {});
  } catch {}
  try { if (typeof renderJournalEntries === 'function') await renderJournalEntries(); } catch {}
